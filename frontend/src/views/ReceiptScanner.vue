      ...
      <div class="scanner-main">
        ...
        <div class="scanner-footer">
          <div class="footer-btns">
            ...
            <button
              :class="['btn-abnormal', { isAbnormal: currentReceipt?.isAbnormal }]"
              @click="markAbnormal"
            >
              {{ currentReceipt?.isAbnormal ? '恢复正常' : '标记异常' }}
            </button>
            <button
              class="btn-nav"
...
// 监听 currentIndex 变化同步 Swiper (如果是通过键盘/其他逻辑切换时)
...
const markAbnormal = () => {
  ...
  const newStatus = !currentReceipt.value.isAbnormal;
  updateReceipt(currentReceipt.value.id, { isAbnormal: newStatus });
  ...
  receipts.value = receipts.value.map((r) =>
    r.id === currentReceipt.value.id ? { ...r, isAbnormal: newStatus } : r
  );
  
  // 只有在标记为“异常”时才自动跳到下一张，恢复正常时留在当前页
  if (newStatus && currentIndex.value < receipts.value.length - 1) {
    currentIndex.value++;
  }
};
...
const handleKeyDown = (e) => {
  ...
  if (e.key === 'ArrowLeft' && currentIndex.value > 0) prevSlide();
...
.btn-abnormal.isAbnormal {
  background: #10b981; /* 变绿表示点击即可恢复正常 */
